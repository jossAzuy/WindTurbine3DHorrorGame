...
Version 5.3.1
- Added "Base Altitude Clip" toggle. When enabled, base altitude acts as a hard clipping plane preventing fog from rendering below it. Disabled by default to preserve previous behavior.

Version 5.3
- Added "Start Distance" option. Allows to start the fog effect at a specific distance from the camera.
- Density Exponential renamed to "Height Spread".

Version 5.2.2
- [Fix] Sub-volume blending now properly carries over all profile properties (depthClip, useXYZDistance, useVerticalGradient, useSunColor, noiseDistanceAtten)

Version 5.2.1
- Added inspector warning if Depth Texture Mode is set to After Transparents (should be After Opaques or Depth PrePass)

Version 5.2
- Added shader optimizations when vertical gradient or light diffusion are not used

Version 5.1.2
- Minor fixes for Unity 2023.3
